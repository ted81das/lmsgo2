...
<head>
    ...
    <title>
        ...
        @if(trim($__env->yieldContent('page-title')) && Auth::user()->type == 'admin')
            {{ config('app.name', 'LMSGo-SaaS') }}
        @else
            {{ isset($currantWorkspace->company) && $currantWorkspace->company != '' ? $currantWorkspace->company : config('app.name', 'LMSGo-SaaS') }}
        @endif
    </title>
...
<script src="{{ asset('/js/jquery.timeentry.js')}}"></script>
