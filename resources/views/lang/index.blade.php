...
@section('action-btn')
    ...
    <div class="btn btn-sm btn-primary btn-icon m-1 float-end">
        <a href="#" class="" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Create Language') }}" data-ajax-popup="true" data-size="md" data-title="{{ __('Create Language') }}" data-url="{{ route('create.language') }}">
    ...
    @if($currantLang != (!empty(env('default_language')) ? env('default_language') : 'en'))
        <div class="action-btn btn btn-sm btn-danger btn-icon m-1 float-end ms-2">
            <form method="POST" action="{{ route('lang.destroy', $currantLang) }}" id="delete-form-{{ $currantLang }}">
                ...
                @method('DELETE')
                @can('delete language')
                    <button type="submit" class="mx-3 btn btn-sm align-items-center show_confirm" data-toggle="tooltip" title='Delete'>
