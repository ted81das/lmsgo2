    ...
    <div class="wrapper">
            ...
            <div class="chapter-head">
                <div class="container offset-right">
                    <div class="chapter-head-content">
                        ...
                        @if ($current_chapter->id != $last_previous->id)
                            <div class="chapter-head-block page-btn-block">
                                <a href="{{ route('store.fullscreen', [$slug, \Illuminate\Support\Facades\Crypt::encrypt($course_id), \Illuminate\Support\Facades\Crypt::encrypt($current_chapter->id), 'previous']) }}"
                                    class="prev-btn">
                                    <span class="svg-ic">
            ...
            <div class="container">
                <div class="row">
                    <div class="col-lg-7 col-12 chapter-video">
                        <div class="{{!empty($current_chapter->type == 'Video File' || $current_chapter->type == 'Text Content')? $current_chapter->type == 'Video File':'chapter-video-media'}}">
                                ...
                                @foreach ($headers as $key => $header)
                                    <li class="set has-children">
                                        <a href="javascript:;" class="acnav-label"
                                            data-bs-target="#ChapterDetails{{ $key }}">
                                                ...
                                                <div class="chapter-topics">
                                                    ...
                                                    <div class="checkbox-custom">
                                                        ...
                                                        @if (isset($chapter_status_check) && $chapter_status_check->status == 'Active')
                                                            <input type="checkbox"
                                                                class="description{{ $i }} checkbox"
                                                                id="description{{ $i }}"
                                                                data-value="{{ $chapters->id }}" checked>
                                                        @else
                                                            <input type="checkbox"
                                                                class="description{{ $i }} checkbox"
                                                                id="description{{ $i }}"
                                                                data-value="{{ $chapters->id }}">
                                                        @endif
                                                        <!-- <input class="description checkbox">  -->
                                                        <label for="description{{ $i }}">{{ $i . '. ' . $chapters->name }}</label>
                                                        {{-- @dd($chapters, $slug, $i, $course_id) --}}
                                                    </div>
                        ...
                        <div class="pdp-tab tabs-wrapper">
                            <ul class="tabs d-flex">
                                <li class="tab-link active" data-tab="tab-1">
                                    <a href="#overview">{{ __('Description') }} </a>
                                </li>
                                <li class="tab-link" data-tab="tab-2">
                                    <a href="#profile">{{ __('Syllabus') }} </a>
                                </li>
                            </ul>
                                ...
                                <div class="tab-content" id="tab-2">
                                    @foreach ($headers as $key => $header)
                                        <div class="set has-children">
                                            <a href="javascript:;" class="acnav-label"
                                                data-bs-target="#ChapterOne{{ $key }}" aria-expanded="true"
                            ...
                            @foreach ($practices_files as $practices_file)
                                <div class="course-download-card" data-id="{{ $practices_file->id }}">
                                    <div class="course-image">
                                        <img src="{{ asset('assets/themes/theme2/images/pdf-image.png') }}" alt="">
                                    </div>
                                    <div class="corse-caption">
                                        <div class="caption-left">
                                            <h5>
                                                <a class="action-item"
                                                    href="{{ asset(Storage::url('uploads/practices/' . $practices_file->files)) }}"></a>
                                            </h5>
        ...
        @php
            ...
            if (!empty($getStoreThemeSetting[$main_homepage_email_subscriber_key])) {
                $homepage_header = $getStoreThemeSetting[$main_homepage_email_subscriber_key];
                $email_subscriber_enable = $homepage_header['section_enable'];

                $homepage_email_subscriber_title_key = array_search('Title', array_column($homepage_header['inner-list'], 'field_name'));
                $homepage_email_subscriber_title = $homepage_header['inner-list'][$homepage_email_subscriber_title_key]['field_default_text'];

                $homepage_email_subscriber_subtext_key = array_search('Sub Text', array_column($homepage_header['inner-list'], 'field_name'));
                $homepage_email_subscriber_subtext = $homepage_header['inner-list'][$homepage_email_subscriber_subtext_key]['field_default_text'];

                $homepage_email_subscriber_button_key = array_search('Button', array_column($homepage_header['inner-list'], 'field_name'));
                $homepage_email_subscriber_button = $homepage_header['inner-list'][$homepage_email_subscriber_button_key]['field_default_text'];
            }
        @endphp
                        ...
                        <div class="newsletter-form">
                            {{ Form::open(['route' => ['subscriptions.store_email', $store->id], 'method' => 'POST']) }}
                                <div class="input-wrapper">
                                    ...
                                    <button type="submit" class="btn btn-white">{{ $homepage_email_subscriber_button }} <svg
                                            viewBox="0 0 10 5">
