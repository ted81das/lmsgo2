...
@section('content')
    <div class="wrapper">
        <section class="common-banner-section"
            style="background-image:url({{ asset('assets/themes/theme2/images/comman-banner.png') }});">
            <div class="container">
                <div class="row">
                    <div class="col-lg-5 col-md-6">
                        <div class="common-banner-content">
                            ...
                            <p>{{ __('Lorem Ipsum is simply dummy text of the printing and typesetting industry.Lorem Ipsum is simply dummy text of the printing and typesetting industry.') }}
                            </p>
                        </div>
                    </div>
                </div>
                                    ...
                                    <div class="col-lg-3 col-md-6 col-sm-6 col-12 product-card course-degree">
                                        <div class="product-card-inner">
                                            ...
                                            <div class="product-img">
                                                <a
                                                    href="{{ route('store.viewcourse', [$slug, \Illuminate\Support\Facades\Crypt::encrypt($course->id)]) }}">
                                                    {{-- <img src="{{ asset('assets/themes/theme2/images/product-1.jpg') }}" alt=""> --}}
                                                    @if (!empty($course->thumbnail))
                                                        <img alt="card"
                                                            src="{{ asset(Storage::url('uploads/thumbnail/' . $course->thumbnail)) }}"
                                                            class="img-fluid">
                                                    @else
                                                        <img src="{{ asset('assets/themes/theme2/images/bachelor-of-sci.jpg') }}"
                                                            alt="">
                                                        {{-- <img src="{{ asset('assets/themes/theme2/images/product-1.jpg') }}" alt=""> --}}
                                                    @endif
                                                </a>
                                                ...
                                                <div class="subtitle-top d-flex align-items-center justify-content-between">
                                                    <span
                                                        class="badge">{{ !empty($course->category_id) ? $course->category_id->name : '' }}</span>

                                                    <div class="btn-wrapp">
                                                        @if (Auth::guard('students')->check())
                                                            @if (sizeof($course->student_wl) > 0)
                                                                <a href="javascript:void(0)"
                                                                    class="wishlist-btn remove_from_wishlist"
                                                                ...
                                                                {!! Form::open([
                                                                    'method' => 'POST',
                                                                    'route' => ['student.removeFromWishlist', [$slug, $course->id]],
                                                                    'id' => 'delete-product-cart-' . $course->id,
                                                                ]) !!}
                                                                {!! Form::close() !!}
                                                            @else
                                                                <a href="#" class="wishlist-btn add_to_wishlist"
                                                                    data-id="{{ $course->id }}">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" width="18"
                                                                        height="18" viewBox="0 0 18 18" fill="none">
                                                                        <path fill-rule="evenodd" clip-rule="evenodd"
                                            ...
                                            <div class="product-content">
                                                <div class="product-content-top">
                                                    <div class="review-rating">
                                                        ...
                                                        @php
                                                            $tutor_course_count = App\Models\Course::where('store_id', $store->id)
                                                                ->where('created_by', $course->created_by)
                                                                ->where('status', 'Active')
                                                                ->get();
                                                            $tutor_id = $tutor_course_count->pluck('created_by')->first();
                                                            $tutor_count_rating = App\Models\Ratting::where('tutor_id', $tutor_id)
                                                                ->where('course_id', $course->id)
                                                                ->where('slug', $store->slug)
                                                                ->where('rating_view', 'on')
                                                                ->count();
                                                        @endphp
                                                    ...
                                                    <h4>
                                                        <a
                                                            href="{{ route('store.viewcourse', [$store->slug, \Illuminate\Support\Facades\Crypt::encrypt($course->id)]) }}">{{ $course->title }}</a>
                                                    </h4>
                                                    @if ($course->course_description)
                                                        <div class="description">{!! $course->course_description !!}</div>
                                                    @else
                                                ...
                                                <div class="product-content-bottom">
                                                    <div class="course-detail">
                                                        <p>
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="12"
                                                                ...
                                                                <defs>
                                                                    <clipPath id="">
                                                                        <rect width="11.7807" height="11.7807"
                                                                            fill="white">
                                                                        </rect>
                                                                    </clipPath>
                                                                </defs>
                                                            </svg>{{ $course->student_count->count() }} &nbsp;
                                                        ...
                                                        <p>
                                                            ...
                                                            </svg> {{ $course->chapter_count->count() }} &nbsp;
                                                            <span>{{ __('Chapters') }}</span>
                                                        </p>
                                                        <p>{{ $course->level }}</p>
                                                    </div>
                                                    ...
                                                    @if (Auth::guard('students')->check())
                                                        @if (in_array(
                                                                $course->id,
                                                                Auth::guard('students')->user()->purchasedCourse()))
                                                            <div class="card-price">
                                                            </div>
                                                            <a class="btn cart-btn cart-btn"
                                                                href="{{ route('store.fullscreen', [$store->slug, Crypt::encrypt($course->id), '']) }}">
                                                                {{ __('Get Started') }}
                                                        ...
                                                        @else
                                                            <div class="card-price">
                                                                @if ($course->has_discount == 'on')
                                                                    <ins>
                                                                        {{ $course->is_free == 'on' ? __('Free') : Utility::priceFormat($course->price) }}</ins>
                                                                @else
                                                                    <ins>
                                                                        {{ $course->is_free == 'on' ? __('Free') : Utility::priceFormat($course->price) }}</ins>
                                                                @endif
                                                            ...
                                                            <div class="add-cart">
                                                                @if ($key !== false)
                                                                    <a id="cart-btn-{{ $course->id }}"
                                                                        class="btn cart-btn cart-btn">{{ __('Already in Cart') }}
                                                                        <svg viewBox="0 0 10 5">
                                                                            <path
                                                                                d="M2.37755e-08 2.57132C-3.38931e-06 2.7911 0.178166 2.96928 0.397953 2.96928L8.17233 2.9694L7.23718 3.87785C7.07954 4.031 7.07589 4.28295 7.22903 4.44059C7.38218 4.59824 7.63413 4.60189 7.79177 4.44874L9.43039 2.85691C9.50753 2.78197 9.55105 2.679 9.55105 2.57146C9.55105 2.46392 9.50753 2.36095 9.43039 2.28602L7.79177 0.69418C7.63413 0.541034 7.38218 0.544682 7.22903 0.702329C7.07589 0.859976 7.07954 1.11192 7.23718 1.26507L8.1723 2.17349L0.397965 2.17336C0.178179 2.17336 3.46059e-06 2.35153 2.37755e-08 2.57132Z">
                                                                            </path>
                                                                ...
                                                                @else
                                                                    <a id="cart-btn-{{ $course->id }}"
                                                                        class="btn cart-btn add_to_cart"
                                                                        data-id="{{ $course->id }}">{{ __('Add in Cart') }}
                                                                        <svg viewBox="0 0 10 5">
                                                                            <path
                                                                                d="M2.37755e-08 2.57132C-3.38931e-06 2.7911 0.178166 2.96928 0.397953 2.96928L8.17233 2.9694L7.23718 3.87785C7.07954 4.031 7.07589 4.28295 7.22903 4.44059C7.38218 4.59824 7.63413 4.60189 7.79177 4.44874L9.43039 2.85691C9.50753 2.78197 9.55105 2.679 9.55105 2.57146C9.55105 2.46392 9.50753 2.36095 9.43039 2.28602L7.79177 0.69418C7.63413 0.541034 7.38218 0.544682 7.22903 0.702329C7.07589 0.859976 7.07954 1.11192 7.23718 1.26507L8.1723 2.17349L0.397965 2.17336C0.178179 2.17336 3.46059e-06 2.35153 2.37755e-08 2.57132Z">
                                                                            </path>
                                                    ...
                                                    @else
                                                        ...
                                                        <div class="add-cart">
                                                            @if ($key !== false)
                                                                <a id="cart-btn-{{ $course->id }}"
                                                                    class="btn cart-btn">{{ __('Already in Cart') }}</a>
                                                            @else
                                                                <a id="cart-btn-{{ $course->id }}"
                                                                    class="btn cart-btn add_to_cart"
                                                                    data-id="{{ $course->id }}">{{ __('Add in Cart') }}</a>
                                                            @endif
                                                        </div>
                                                    @endif
                                                </div>
                    ...
                    <div class="newsletter-container">
                        ...
                        <div class="newsletter-form">
                            ...
                            <div class="checkbox-custom">
                                ...
                                <label
                                    for="newslettercheckbox">{{ __('Lorem Ipsum is simply dummy text of the printing and typesetting industry.') }}</label>
                            </div>
                            {{ Form::close() }}
                        </div>
                    </div>
...
@push('script-page')
    ...
    <script>
        $(document).on('click', '.add_to_cart', function(e) {
            ...
            $.ajax({
                type: "POST",
                url: '{{ route('user.addToCart', ['__product_id', $store->slug]) }}'.replace('__product_id',
                    id),
                data: {
                    "_token": "{{ csrf_token() }}",
                },
    ...
    <script>
        $(document).on('click', '.add_to_wishlist', function(e) {
            ...
            var _url;
            if (id_2 == null) {
                _url = '{{ route('student.addToWishlist', [$store->slug, '__course_id']) }}'.replace('__course_id',
                    id);
            } else {
                _url = '{{ route('student.addToWishlist', [$store->slug, '__course_id']) }}'.replace('__course_id',
                    id_2);
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status == "Success") {
                        show_toastr('Success', response.success, 'success');
                        if (id_2 == null) {
                            $('.fygyfg_' + id).children().attr('src',
                                '{{ asset('assets/img/wishlist.svg') }}');
                        } else {
                            $('.fygyfg_' + id_2).children().attr('src',
                                '{{ asset('assets/img/wishlist.svg') }}');
                        }
