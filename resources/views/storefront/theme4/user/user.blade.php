...
@endphp
@include('layouts.theme4.shophead')
@php
...
@endphp
<body>
@include('layouts.theme4.shopheader')
    ...
    @if(!empty($getStoreThemeSetting[5]) && $getStoreThemeSetting[5]['section_enable'] == 'on')
                    ...
                    @if($demoStoreThemeSetting['enable_footer_note'] == 'on')
                        <div class="footer-logo footer-col">
                            <a href="{{route('store.slug',$store->slug)}}">
                                @if(!empty($getStoreThemeSetting[5]['inner-list'][0]['field_default_text']))
                                    <img src="{{asset(Storage::url('uploads/'.$getStoreThemeSetting[5]['inner-list'][0]['field_default_text']))}}" alt="lmsgo-logo">
                                @else
                                    <img src="{{asset(Storage::url('uploads/theme4/footer/lmsgo-logo.png'))}}" alt="Footer logo">
                                @endif
                            </a>
                        </div>
                        <div class="footer-col footer-link footer-link-1">
                            <div class="footer-widget">
                                <ul>
                                    @for ($i = 0 ;  $i < $getStoreThemeSetting[6]['loop_number'] ; $i++)
                                        @php
                                            $a = '#';
                                            $footer1_name = '';
                                        @endphp
                                        @foreach ($getStoreThemeSetting[6]['inner-list'] as $item_key => $item)
                                            @php
                                                if ($item_key == 0) {
                                                    if(!empty($getStoreThemeSetting[6][$item['field_slug']])) {
                                                        $footer1_name = $getStoreThemeSetting[6][$item['field_slug']][$i];
                                                    } else {
                                                ...
                                                if ($item_key == 1) {
                                                    if(!empty($getStoreThemeSetting[6][$item['field_slug']])) {
                                                        $a = $getStoreThemeSetting[6][$item['field_slug']][$i];
                                                    } else {
                                            ...
                                            @php
                                                if ($item_key == 0) {
                                                    if(!empty($getStoreThemeSetting[7][$item['field_slug']])) {
                                                        $icon = $getStoreThemeSetting[7][$item['field_slug']][$i];
                                                    } else {
                                                ...
                                                if ($item_key == 1) {
                                                    if(!empty($getStoreThemeSetting[7][$item['field_slug']])) {
                                                        $link = $getStoreThemeSetting[7][$item['field_slug']][$i];
                                                    } else {
...
{{-- <script src="{{asset('js/lmscustom.js')}}" ></script> --}}
    ...
    <script src="{{ asset('assets/themes/theme4/js/slick.min.js') }}"></script>
    <script src="{{ asset('assets/themes/theme4/js/custom.js') }}"></script>
